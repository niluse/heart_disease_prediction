...
# Training and evaluating kNN model with different k values
...
# Setting the range of k values for testing
...
# 7.1.1 Evaluating the model's performance
...
# 7.1.2 Evaluating the model's performance
conf_matrix <- table(kNN_model, test_set$target)
accuracy <- sum(diag(conf_matrix))/sum(conf_matrix)
precision <- conf_matrix[2, 2] / sum(conf_matrix[, 2])
recall <- conf_matrix[2, 2] / sum(conf_matrix[2, ])
...
# Converting predictions to binary using a threshold of 0.5
svm_predictions_binary <- ifelse(svm_predictions >= 0.5, 1, 0)

# 7.2 Evaluating the model's performance
...
# 9.2 Visualizing Classifier Results
# ROC Curve for SVM
...
# 7.2.2 Evaluating the model's performance
conf_matrix <- table(Actual = test_set$target, Predicted = svm_predictions_binary)
accuracy <- sum(diag(conf_matrix))/sum(conf_matrix)
precision <- conf_matrix[2, 2] / sum(conf_matrix[, 2])
recall <- conf_matrix[2, 2] / sum(conf_matrix[2, ])
...
# 7.3.1 Evaluating the model's performance
...
# 7.3.2 Evaluating the model's performance
conf_matrix <-  table(Actual = test_set$target, Predicted = tree_predictions)
accuracy <- sum(diag(conf_matrix))/sum(conf_matrix)
precision <- conf_matrix[2, 2] / sum(conf_matrix[, 2])
recall <- conf_matrix[2, 2] / sum(conf_matrix[2, ])
...
# 7.4.1 Evaluating the model's performance
...
# 7.4.2 Evaluating the model's performance
conf_matrix <-  table(Actual = test_set$target, Predicted = rf_predictions_binary)
accuracy <- sum(diag(conf_matrix))/sum(conf_matrix)
precision <- conf_matrix[2, 2] / sum(conf_matrix[, 2])
recall <- conf_matrix[2, 2] / sum(conf_matrix[2, ])
...
# 7.5.1 Evaluating the model's performance
...
# 9.5.2 Visualizing Classifier Results
# ROC Curve for XGBoost
roc_curve_boost <- roc(test_set$target, y_pred_binary, levels = c(0, 1))
plot(roc_curve_boost, col = "blue", main = "ROC Curve (XGBoost)", col.main = "darkblue")
# Precision-Recall Curve 
